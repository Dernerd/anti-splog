	...
	class Puc_v4p10_Plugin_Ui {
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->updateChecker->slug) ) {
				...
				if ( $status == 'no_update' ) {
					$message = sprintf(_x('Das %s-Plugin ist laut PSource Server auf dem neuesten Stand.', 'the plugin title', 'ust'), $title);
				} else if ( $status == 'update_available' ) {
					$message = sprintf(_x('Eine neue Version des %s-Plugins ist auf dem PSource Server verfügbar.', 'the plugin title', 'ust'), $title);
				} else if ( $status === 'error' ) {
					$message = sprintf(_x('Es konnte nicht festgestellt werden, ob Updates für %s verfügbar sind.', 'the plugin title', 'ust'), $title);
					$noticeClass = 'error notice-error';

					$details = $this->formatManualCheckErrors(get_site_transient($this->manualCheckErrorTransient));
					delete_site_transient($this->manualCheckErrorTransient);
				} else {
					$message = sprintf(__('Unbekannter Update-Status "%s"', 'ust'), htmlentities($status));
					$noticeClass = 'error notice-error';
				}
				printf(
